...
import { getCmsToken, getOrgIdToken, setCmsToken } from '@/config/token'
...
import { useQueryGetCompanyUserLogin } from '@/service/resource/company/userLogin'
import { useQueryGetOrgAllByUser } from '@/service/resource/org/getAllByUser'
import { useQueryGetThemeConfigAPI } from '@/service/resource/theme/get'
...
export const useLayout = () => {
  ...
  useEffect(() => {
    try {
      if (!orgName) {
        const orges = orgList?.data ?? []
        if (orges.length > 0) {
          handleChangeOrg(orges[0].id, orges[0].name).then(() => {
            dispatch(
              setOrgConfig({
                id: orges[0].id,
                name: orges[0].name,
